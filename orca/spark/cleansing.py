...
def dedupsByRank(
    ...
    windowBy = Window.partitionBy(
        *list(map(lambda col_name: col(col_name), partitionBy))
    ).orderBy(*list(map(lambda col_name: _(col_name), orderCols)))
    return (
        df.withColumn("rank", rankType.over(windowPartition))
        .filter(col("rank") == 1)
        .drop("rank")
    )
